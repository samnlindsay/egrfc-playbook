---
layout: default
title: Team plays
permalink: /attack/team-plays/
parent: Attack
nav_order: 13
---

# Team Plays
{: .no_toc }

## Table of Contents
{: .no_toc .text-delta }

- TOC
{:toc}

Rather than playing "heads up" from phase to phase, sometimes it is useful to have one or several phases pre-planned to help re-establish our attacking shape and break down a well-organised defence.

This section contains plays that involve both forwards and backs working together in coordinated moves, including both single-phase set plays and multi-phase sequences.

---

# Single-phase plays

## Blackjack

A pre-planned move to quickly shift the point of attack away from an edge while the defence reorganises. Midfield pod of 3 forwards stand in a line, offering all 3 as passing options for the playmaker and ready to generate quick ball for the next pod.

<img src="{{ '/assets/images/attack/blackjack.png' | relative_url }}" style="width:100%; max-width: 600px; height: auto;"/>

## Poker

A pre-planned move out the back of a queen pod, designed to stretch the defence and give the playmaker 3 passing options:
1. A switch to a hard running forward cutting back inside
2. A tip to a hard running forward on the outside
3. Releasing the outside backs

All three options can potentially break the gain line (as can the playmaker running it themselves), depending on how the defence reacts.

<img src="{{ '/assets/images/attack/poker.png' | relative_url }}" style="width:100%; max-width: 600px; height: auto;"/>

---

# Multi-phase plays

## Fanta

Using the 1-3-3-1 pod structure to create multiple phases of play from edge to edge, focusing on challenging C/D defenders.

#### Phase 1
King take-off to a second pod off 10
<img src="{{ '/assets/images/attack/fanta1.png' | relative_url }}" style="width:100%; max-width: 600px; height: auto;"/>

#### Phase 2
King carry around the corner
<img src="{{ '/assets/images/attack/fanta2.png' | relative_url }}" style="width:100%; max-width: 600px; height: auto;"/>

#### Phase 3 
King take-off back infield. 10 feeds 15 playing to the edge behind a decoy line from 2
<img src="{{ '/assets/images/attack/fanta3.png' | relative_url }}" style="width:100%; max-width: 600px; height: auto;"/>
#### Phase 4
King take-off to a second pod off 10 (similar to phase 1)
<img src="{{ '/assets/images/attack/fanta4.png' | relative_url }}" style="width:100%; max-width: 600px; height: auto;"/>

## Cola

Four phases off a hot 4-man lineout, playing to the opposite edge and back, making use of decoy runners.

#### Phase 1 

8-12 carry round the back of the lineout
<img src="{{ '/assets/images/attack/cola1.png' | relative_url }}" style="width:100%; max-width: 600px; height: auto;"/>

#### Phase 2
10-15 play to the opposite edge behind 13 and 7 running decoy lines
<img src="{{ '/assets/images/attack/cola2.png' | relative_url }}" style="width:100%; max-width: 600px; height: auto;"/>

#### Phase 3 
Queen pod off 12 back infield
<img src="{{ '/assets/images/attack/cola3.png' | relative_url }}" style="width:100%; max-width: 600px; height: auto;"/>

#### Phase 4
Queen take-off to 13 with 1 running a decoy line in front of 10
<img src="{{ '/assets/images/attack/cola4.png' | relative_url }}" style="width:100%; max-width: 600px; height: auto;"/>

---